...
module phase_accumulator(
    ...
    input wire [31:0] ftw,              // frequency tuning word calculated from C program sent via LW_AXI bus
    input wire rst,                     // master reset switch coming from on board button (will have to update this)
    output reg [31:0] accumulated_phase // output value from phase accumulator
    );

    always@(posedge clk_150mhz or posedge rst) begin
        if (rst) begin
            accumulated_phase <= 32'b0;
        end else begin
