...
module phase_accumulator(
    ...
    input wire [28:0] ftw,              // frequency tuning word calculated from C program sent via LW_AXI bus
    input wire phase_rst,               // trigger to reset phase before pulsing to start pulse with no phase offset
    output reg [28:0] accumulated_phase // output value from phase accumulator
    );

    // accumulated_phase FF logic
    always@(posedge clk_150mhz or posedge rst) begin
        if (rst) begin
            accumulated_phase <= 29'd0;
        end else begin
            if (phase_rst) begin
                accumulated_phase <= 29'd0;
            end else begin
